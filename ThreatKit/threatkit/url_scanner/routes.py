...
def page():
    ...
    if request.method == "POST":
        ...
        if url:
            ...
            try:
                prompt = f"You are a cybersecurity assistant. Analyze the following URL and its heuristics report for potential risks. The heuristics results either trigger (explains the issue), or states 'No issues detected'. Look for misspellings, suspicious domain endings, unusual subdomains, use of URL shorteners, HTTP or HTPPS, or misleading keywords. Respond with the shortest possible educational explanation as to why it may or may not be safe. Always respond in the following format: 'This link is likely Suspicious/Safe' On a new line, no more than six reasons formatted as, '1. <why suspicious/safe reason 1>'. On a new line, brief explanation summarizing points made.\n{report}"
                with requests.post(
                    OLLAMA_URL,
