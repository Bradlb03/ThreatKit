    ...
    if request.method == "POST":
        ...
        if pw:
            try:
                ...
                # store password only for later AI call (not shown in UI)
                result["input_password"] = pw
...
def ai_summary():
    ...
    if not pw:
        return jsonify({"summary": "Missing password"}), 400
    ...
    try:
        ...
        return jsonify({"summary": f"Error contacting AI model: {e}"}), 500