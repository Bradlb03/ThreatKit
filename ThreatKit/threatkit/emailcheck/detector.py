...
def analyze_email(subject, from_hdr, return_path, to_hdr, body, headers: Optional[Dict] = None) -> Dict:
    ...
    if ML_AVAILABLE:
        try:
            ...
            # Smooth the ML probability (less aggressive than before)
            p_ml = _smooth_phishing_prob(phishing_prob, temperature=0.9)

            # Convert rule sum into a probability-like signal (score-aware)
            p_rule = 1.0 - math.exp(-rule_sum / 10.0)  # stronger curve so moderate rule sums matter more

            # Blend: give rules more weight than before to increase recall on rules-based signals
